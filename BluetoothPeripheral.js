...
var SERVICE_UUID = 'B8E06067-62AD-41BA-9231-206AE80AB550'; // 'fc00'
var SERVICE_NAME = 'TCup'; // 'feedback'
var CHARACTERISTIC_UUID = 'BF45E40A-DE2A-4BC8-BBA0-E5D6065F1B4B'; // 'fc0f'

bleno.on('stateChange', function(state) {
  console.log('on -> stateChange: ' + state);
  if (state === 'poweredOn') {
    bleno.startAdvertising(SERVICE_NAME, [SERVICE_UUID]);
  }   
  else {
  ...
  if (!error) {
    bleno.setServices([
      new BlenoPrimaryService({
        uuid: SERVICE_UUID,
        characteristics: [
          new BluetoothCharacteristic(CHARACTERISTIC_UUID)
        ]
      })
    ]);
  }
